...
def function_to_choose_fragmentation(fragmentations):
    ...
    sorted_fragmentations_dict = sorted(
        fragmentations_descriptors.items(), key=operator.itemgetter(1)
    )

    return fragmentations[sorted_fragmentations_dict[0][0]]


def is_fragmentation_equal_to_other_fragmentation(
    fragmentation, other_fragmentation
):
    for group_number, amount in fragmentation.items():
        if group_number in other_fragmentation:
            if (
                fragmentation[group_number]
                != other_fragmentation[group_number]
            ):
                return False
    return True


def log_structure_results(
    f,
    pubchem_id,
    SMILES,
    inchikey,
...
complete_fragmenter = fragmenter(
...
print("####################################################################")
print(
    "Fragmenting the reference database without the patterns sorted (simple and complete algorithm)"
)
...
with (
    ...
    for inchikey, SMILES, pubchem_id, fragmentation_reference_DB in tqdm(
        reference_DB, total=len(reference_DB)
    ):
        lines = []

        for group_number, amount in fragmentation_reference_DB.items():
            if group_number not in reference_DB_fragmentation_stats:
        ...
        fragmentation, success, fragmentation_matches = (
            simple_fragmenter.fragment(SMILES)
        )
        if success:
            simple_fragmenter_fragmented.append(inchikey)
            if is_fragmentation_equal_to_other_fragmentation(
                fragmentation, fragmentation_reference_DB
            ):
                simple_fragmenter_fragmented_and_equal_to_reference_DB.append(
                    inchikey
                )
        ...
        if n_heavy_atoms <= 20:
            ...
            fragmentation, success, fragmentation_matches = (
                complete_fragmenter.fragment(SMILES)
            )
            if success:
                ...
                if is_fragmentation_equal_to_other_fragmentation(
                    ...
                    complete_fragmenter_fragmented_and_equal_to_reference_DB.append(
                        inchikey
                    )
            ...
            log_structure_results(
                ...
                fragmentation,
                fragmentation_reference_DB,
                "Structure was skipped because it is larger than 20 atoms.",
            )
...
print(
    "N_fragmented(simple): "
    + str(len(simple_fragmenter_fragmented))
    + "("
    + str((1.0 * len(simple_fragmenter_fragmented)) / len(reference_DB))
    + ")"
)
print(
    "N_fragmented_and_equal(simple): "
    + str(len(simple_fragmenter_fragmented_and_equal_to_reference_DB))
    + "("
    + str(
        (1.0 * len(simple_fragmenter_fragmented_and_equal_to_reference_DB))
        / len(reference_DB)
    )
    + ")"
)
...
print(
    "N_fragmented(complete): "
    + str(len(complete_fragmenter_fragmented))
    + "("
    + str(
        (1.0 * len(complete_fragmenter_fragmented))
        / len(right_size_for_complete_fragmenter)
    )
    + ")"
)
print(
    "N_fragmented_and_equal(complete): "
    + str(len(complete_fragmenter_fragmented_and_equal_to_reference_DB))
    + "("
    + str(
        (1.0 * len(complete_fragmenter_fragmented_and_equal_to_reference_DB))
        / len(right_size_for_complete_fragmenter)
    )
    + ")"
)
...
complete_fragmenter.fragmentation_scheme_order = (
    sorted_group_numbers_as_in_paper
)
...
print("####################################################################")
print(
    "Fragmenting the reference database with the patterns sorted (simple and complete algorithm)"
)
    ...
    for inchikey, SMILES, pubchem_id, fragmentation_reference_DB in tqdm(
        ...
        if success:
            ...
            if is_fragmentation_equal_to_other_fragmentation(
                ...
                simple_fragmenter_sorted_fragmented_and_equal_to_reference_DB.append(
                    inchikey
                )
        ...
        if n_heavy_atoms <= 20:
            ...
            if success:
                ...
                if is_fragmentation_equal_to_other_fragmentation(
                    ...
                    complete_fragmenter_sorted_fragmented_and_equal_to_reference_DB.append(
                        inchikey
                    )
...
print(
    "N_fragmented(simple): "
    + str(len(simple_fragmenter_sorted_fragmented))
    + "("
    + str((1.0 * len(simple_fragmenter_sorted_fragmented)) / len(reference_DB))
    + ")"
)
print(
    "N_fragmented_and_equal(simple): "
    + str(len(simple_fragmenter_sorted_fragmented_and_equal_to_reference_DB))
    + "("
    + str(
        (
            1.0
...
print("")
print(
    "N_structures(complete):" + str(len(right_size_for_complete_fragmenter2))
)
print(
    "N_fragmented(complete): "
    + str(len(complete_fragmenter_sorted_fragmented))
    + "("
    + str(
        (1.0 * len(complete_fragmenter_sorted_fragmented))
        / len(right_size_for_complete_fragmenter2)
    )
    + ")"
)
print(
    "N_fragmented_and_equal(complete): "
    + str(len(complete_fragmenter_sorted_fragmented_and_equal_to_reference_DB))
    + "("
    + str(
        (
            1.0
...
combined_fragmenter = fragmenter(
    fragmentation_scheme,
    algorithm="combined",
    n_heavy_atoms_cuttoff=20,
    function_to_choose_fragmentation=function_to_choose_fragmentation,
...
combined_fragmenter.fragmentation_scheme_order = (
    sorted_group_numbers_as_in_paper
)
...
print(
    "N_fragmented(simple): "
    + str(len(combined_fragmenter_sorted_fragmented))
    + "("
    + str(
        (1.0 * len(combined_fragmenter_sorted_fragmented)) / len(structures_DB)
    )
    + ")"
)
print("")
